...
public:
    Blur(std::vector<std::string> _image_paths)
    {
    ...
    void exportBlurData(std::string file_name = "data.csv")
    {
        ...
        {
            std::string imageName = image_paths[i];
            ...
            // Progress
            std::cout << "\rProgress: " << (i+1) << "/" << image_paths.size() << std::flush;
        }
